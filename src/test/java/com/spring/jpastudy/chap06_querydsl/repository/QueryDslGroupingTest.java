...
class QueryDslGroupingTest {
    ...
    void groupByGenderTest() {
        ...
        //when
        /*
           SELECT G.group_id, G.group_name, I.gender, COUNT(I.idol_id)
           FROM tbl_idol I
           JOIN tbl_group G
        ...
        List<Tuple> idolList = factory
                ...
                .having(idol.count().loe(3))   // 그룹화한 다음 카운트에서 3 이하인 조건 !
                .fetch();
        //then
        System.out.println("\n\n\n");

        for (Tuple tuple : idolList) {
            Group group = tuple.get(idol.group);// 추출하고 싶은거 넣기 !
    ...
    void ageGroupTest() {
        /*
            SELECT
                  GROUP BY CASE age WHEN BETWEEN 10 AND 19 THEN 10
                  GROUP BY CASE age WHEN BETWEEN 20 AND 29 THEN 20
                  GROUP BY CASE age WHEN BETWEEN 30 AND 39 THEN 30
                  END,
                  COUNT(idol_id)
            FROM tbl_idol
            GROUP BY CASE age WHEN BETWEEN 10 AND 19 THEN 10
        ...
        NumberExpression<Integer> otherwise = new CaseBuilder()
        ...
        for (Tuple tuple : result) {
            int ageGroupValue = tuple.get(otherwise);
            long count = tuple.get(idol.count());
        ...
        @Test
        @DisplayName("아이돌 그룹별로 아이돌의 그룹명과 평균 나이를 조회하세요. 평균 나이가 20세와 25세 사이인 그룹만 조회합니다.")
        void groupBy() {
            //given
    ...
    void groupAverageAgeTest() {
        ...
        // Projections : 커스텀한 dto를 포장해주는 객체
        List<GroupAverageAgeDto> result = factory
                .select(
                        // ⭐⭐select 결과를 dto 타입으로 조회해준다 !
